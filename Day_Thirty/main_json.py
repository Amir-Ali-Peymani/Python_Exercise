...
def search_data():
    with open("D:\Projects\Python\Python_Exercise\Day_Thirty\password.json", "r") as file:
        data = json.load(file)
        website_local = webiste.get()
        if website_local in data:
            email = data[website_local]["email"]
            password = data[website_local]["password"]
            messagebox.showinfo(title=website_local, message=f"Email: {email}\nPassword: {password}")
        else:
            messagebox.showerror(title="Error", message="didn't find anything")
...
def save_data():
    file = open("D:\Projects\Python\Python_Exercise\Day_Thirty\password.json", "w")
    website_local = webiste.get()
...
webiste.focus()
webiste_serach_button = tkinter.Button(text="Search", width=14, command=search_data)
